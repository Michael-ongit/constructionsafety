...
const MAX_CLIENT_HISTORY = 8; // clean (role, content) turns kept client-side
...
export function useChatStream({ onAssistantDone } = {}) {
  ...
  const send = useCallback(
    async (text) => {
      ...
      // History sent to the server is only clean (role, content) text --
      // never SQL/debug data -- from the messages already on screen.
