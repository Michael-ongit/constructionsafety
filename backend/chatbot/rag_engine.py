...
_IS_SQLITE = db.name in {"sqlite", "local", "local_sqlite"}
_DIALECT_NAME = "SQLite" if _IS_SQLITE else "Microsoft SQL Server (T-SQL)"
_ROW_LIMIT_RULE = (
    "- Use LIMIT for row limits; do not use TOP."
    if _IS_SQLITE else
    "- Use TOP for row limits; do not use LIMIT."
)

SQL_SYSTEM_PROMPT_TEMPLATE = f"""Write one valid {_DIALECT_NAME} SELECT query for the question below.

CRITICAL: Use ONLY the exact table and column names shown in the schema context below. Do NOT invent or guess table names. The tables and columns listed are the ONLY ones that exist.
...
- The table for compliance/audit data is called compliance_audit (NOT ComplianceIssues).
{_ROW_LIMIT_RULE}"""

ANSWER_SYSTEM_PROMPT = """You are the Know Harm AI Safety assistant. Answer using ONLY the database results provided.
...
Do NOT use HTML tags like <br>, <b>, <i>, or <p>. Do NOT wrap your response in quotation marks.
Use plain text with line breaks and bullet points for formatting."""

SQL_FIX_PROMPT = f"""Fix the following {_DIALECT_NAME} query based on the database error and schema context.
