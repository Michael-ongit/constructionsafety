...
export default function ChatWidget() {
  ...
  const handleAssistantDone = ({ text, error }) => {
    ...
    notify("Know Harm AI Assistant", body.slice(0, 160));
  };
  ...
  return (
    <>
      <button
        ...
        onClick={handleOpen}
        aria-label="Open Know Harm AI assistant"
      >
        <ChatIcon />
      ...
      <aside className={`ehs-panel ${open ? "ehs-panel--open" : ""}`} role="dialog" aria-label="Know Harm AI assistant">
        <header className="ehs-panel-header">
          <div className="ehs-panel-title">
            <span className={`ehs-dot ${isStreaming ? "ehs-dot--busy" : ""}`} />
            <span>Know Harm AI Assistant</span>
          </div>
